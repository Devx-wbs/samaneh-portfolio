...
const Section_d = () => {
  return (
    <section className="bg-[#F5F2EC] py-24 px-4 flex justify-center">
      <div className="max-w-7xl w-full grid md:grid-cols-2 gap-12 items-center bg-white p-6 sm:p-10 rounded-3xl shadow-xl">

        {/* Right: Image (moved here to be on the left visually) */}
        <motion.div
        ...
        {/* Left: Text content (was right before) */}
        <motion.div
        ...
        >
          <div className="relative pl-6">
            <span className="absolute left-0 top-1 h-full w-1 bg-[#5BC0EB] rounded-full"></span>
            <h2 className="text-4xl md:text-5xl font-extrabold text-[#2A2523] leading-tight" style={{ fontFamily: 'Space Grotesk' }}>
              Our Solution
              <br />
              <span className="text-[#5BC0EB]">(Core Functionality)</span>
            </h2>
          </div>
          ...
          <div className="space-y-6">
            {features.map((feature, i) => (
              <motion.div
                ...
                style={{ backgroundColor: feature.bg, borderLeft: `4px solid ${feature.border}` }}
              >
                <img src={feature.icon} alt={feature.title} className="w-14 h-14 sm:w-16 sm:h-16 object-contain shrink-0" />
                <div>
                  <h3 className="text-xl sm:text-2xl font-bold text-[#2A2523]" style={{ fontFamily: 'Space Grotesk' }}>
                    {feature.title}
                  </h3>
                  <p className="text-sm sm:text-base text-[#2A2523]" style={{ fontFamily: 'Inter' }}>
                    {feature.description}
                  </p>
                </div>
              </motion.div>
